...
$pdf->AddPage();
    ...
    //$pdf->Cell(0,10,'Quotation  ID='.$quotations->id,0,1);
    $pdf->SetFont('Times','B',16);
    $pdf->Cell(0,10,'QUOTATION',0,1,'C');
    $pdf->SetFont('Times','',12);
    $pdf->Cell(0,10,'Date :'.$quotations->quotationDate,0,1);
    $pdf->Cell(0,10,'For the attention of :'.$quotations->customerData->firstName.' '.$quotations->customerData->lastName,0,1);
    $pdf->Cell(0,10,'Address :'.$quotations->customerData->address.' '.$quotations->customerData->city.' '.$quotations->customerData->postcode.' '.$quotations->customerData->state,0,1);
    $pdf->Cell(0,10,'Email :'.$quotations->customerData->email,0,1);
    $pdf->Cell(0,10,'Telefon Number :'.$quotations->customerData->mobile.' '.$quotations->customerData->office.' '.$quotations->customerData->home.' ',0,1);
    $pdf->Cell(0,10,'For patient :'.$quotations->patientData->firstName.' '.$quotations->patientData->lastName,0,1);
    $pdf->Cell(0,10,'Your request :',0,1);
    
    //var $number = 1;
    
    foreach($quotations->noteListData as $quotationNote) {
    	$pdf->MultiCell(0,10,$quotationNote->note,0,1);
    	//$number = $number+1;
    }
